...
suite("User API tests", () => {
  ...
  test("create a user", async () => {
   ...
   test("delete all users", async () => {
    ...
    const firstUser = await db.userStore!.getUserByUsername(testUsers[0].username);
    await db.userStore!.deleteUserById(firstUser!._id!);
    const returnedUsers = await db.userStore!.getAllUsers();
    assert.equal(returnedUsers.length, testUsers.length - 1);
    const deletedUser = await db.userStore!.getUserById(firstUser!._id!);
